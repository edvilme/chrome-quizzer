...
function handleGenerateQuiz(elements) {
  ...
  // Fetch tab data first
  chrome.runtime.sendMessage({ type: 'getTab' }, (tabResp) => {
    if (chrome.runtime.lastError || !tabResp || !tabResp.success) {
      document.body.setAttribute('data-status', 'error');
      return;
    }

    const tabData = tabResp.tabData;

    // Generate summary
    chrome.runtime.sendMessage({ type: 'generateSummary', tabData }, (summaryResp) => {
      if (chrome.runtime.lastError || !summaryResp || !summaryResp.success) {
        document.body.setAttribute('data-status', 'error');
        return;
      }
      ...
      // Generate quiz
      chrome.runtime.sendMessage({ type: 'generateQuiz', tabData }, (quizResp) => {
        if (chrome.runtime.lastError || !quizResp || !quizResp.success) {
          document.body.setAttribute('data-status', 'error');
          return;
        }
        ...
        updatePageContent(elements, {
          favicon: tabData?.favicon,
          article: tabData?.article,
          summary,
          quiz
        });
      });
